...
export const BadgesGallery: React.FC<BadgesGalleryProps> = ({
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      {badges.length === 0 ? (
        <Paper sx={{ p: 5, textAlign: 'center', bgcolor: 'background.default', border: 1, borderColor: 'divider' }}>
          <VerifiedIcon sx={{ fontSize: 48, color: 'text.primary', mb: 1 }} />
          <Typography variant="h6" sx={{ color: 'text.secondary' }}>No skill badges earned yet</Typography>
          <Typography variant="body2" sx={{ color: 'text.secondary' }}>Take an assessment and achieve a passing score to earn your verified badge.</Typography>
        </Paper>
      ) : (
          ...
          {badges.map((badge) => {
            ...
            return (
              <Grid item xs={12} sm={6} md={4} key={badge.id}>
                <Card sx={{ bgcolor: 'background.paper', border: `2px solid ${tierStyle.border}`, borderRadius: 2.5, position: 'relative', height: '100%', display: 'flex', flexDirection: 'column' }}>
                  <CardContent sx={{ p: 3, flexGrow: 1, display: 'flex', flexDirection: 'column', textAlign: 'center' }}>
                    ...
                    <Box sx={{ bgcolor: 'background.default', p: 1.5, borderRadius: 1.5, mb: 2, border: 1, borderColor: 'divider' }}>
                      <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block' }}>
                        Verification Code:
                      </Typography>
                      <Typography variant="body2" fontWeight="bold" sx={{ color: 'primary.main', fontFamily: 'monospace' }}>
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, justifyContent: 'center', mb: 2, flexGrow: 1 }}>
                      {badge.skills_validated?.map((skill, idx) => (
                        <Chip
                          ...
                          label={skill}
                          size="small"
                          sx={{ bgcolor: 'background.default', color: 'text.secondary', fontSize: '0.7rem', border: 1, borderColor: 'divider' }}
                        />
                      ))}
                    </Box>
