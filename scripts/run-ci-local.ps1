...
if (-not (Test-Path ".env.example")) {
    ...
    $script:FailedSteps += "Backend Environment Audit"
} else {
    ...
    if ($missingKeys.Count -eq 0) {
        ...
        Write-Host "[FAILED]: backend/.env.example missing: $($missingKeys -join ', ')" -ForegroundColor Red
        $script:FailedSteps += "Backend Environment Audit"
    }
}

Run-Step "Backend Go Module Verification" { go mod verify }
Run-Step "Backend Unit & Integration Tests" { $env:ALLOW_NO_DB="true"; go test ./... }
...
if (-not (Test-Path ".env.example")) {
    ...
    $script:FailedSteps += "Frontend Environment Audit"
} else {
...
# ----------------------------------------------------
Write-Host "`n--- [3/3] Security and Vulnerability Audit ---" -ForegroundColor Cyan
Set-Location "$RootDir\frontend"
