...
- Unit-harness follow-up: the stall was traced to an unstable mocked authentication value that retriggered the apply modal's `[open, user]` initialization effect on every render. The modal test now uses a provider-stable user, runs in its own file, and the application/modal regression passed 13/13. A full unsharded loaded run then reached 538/539 and exposed one MUI exit-transition assertion exceeding Testing Library's one-second default; after allowing five seconds for that real transition, the focused company-review file passed 13/13. The required eight CI shards remain the authoritative complete-suite result for the pushed SHA.
