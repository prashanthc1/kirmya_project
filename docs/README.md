...
- [**Workspace Authority Audit**](architecture/27-workspace-authority-audit.md): where every workspace entitlement actually comes from in the current code and schema, the eligibility rule for each, and the ordered migration map for implementing the resolver.
