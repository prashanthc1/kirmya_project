...
| `jobs-recruiting` | **E2E / Jobs & Recruiting** | `jobs.spec.ts`<br>`recruiter-journey.spec.ts`<br>`recruiter-pipeline.spec.ts` | chromium | The job board and its filters, and the recruiter journey against real postings, applicants and applications. |
