...
| F19 | P1 | verified | Platform and QA lead | Release gates / 2 | Frontend release checks verified green: ESLint (0 errors), TypeScript (0 errors), Vitest (567/567 pass), Next.js production build clean. |
| F20 | P1 | open | Technical lead | Evidence governance / 1, 12 | Audit scores and operational claims exceed their evidence. Core beta verified; full platform release hold remains until remaining domains are completed. |
...
| R02 | P2 | open | Backend security lead | Identity and access / 3 | Access-token session revocation after password reset. **Partly closed 7 Sep 2026:** the discarded `RevokeAllUserSessions` error is fixed — the password change and session revocation are now one transaction, and reuse detection no longer claims a revocation it did not perform (`62c8347`, `98c5c78`). Still open: an issued access token stays valid for its remaining 15-minute lifetime because nothing on the request path consults revocation. Closing that changes the authentication architecture and needs a product decision. See [batch 2 evidence, 7 Sep 2026](../BATCH2_DELIVERY_EVIDENCE_2026-09-07.md) |
