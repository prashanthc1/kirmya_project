...
not against the document that last claimed it. All 22 findings from the September 5 audit and the batches since are closed. Four more were found by the batch 6 acceptance pass and the work that followed it: F23, F24, F25 and F26 are all closed. F24 was the release blocker; closing it uncovered F25 and F26, which are the same defect on surfaces and services F24 did not enumerate, and both are closed on the same evidence. Two
...
| F24 | P1 | verified | Frontend experience lead with hiring and admin domain leads | Full-platform domain acceptance / 10A, 10B, 10F | Fourteen files under `frontend/src` presented invented people as real; ten made no API call at all, so reachable authenticated pages rendered fiction. Found and **fixed 9 Sep 2026**: all 120 fabricating catch blocks removed from 14 API clients, every surface now reads from the API or states that it cannot, and `networkingApi` lost its 456-line `mockState`. The one file still listed in the quarantine is a test seam behind `isTestEnv` that cannot render. Guarded by `frontend/src/test/invented-identities.test.ts` and `frontend/src/test/api-clients-do-not-invent.test.ts`. [Batch 6 acceptance](../BATCH6_RELEASE_ACCEPTANCE_2026-09-09.md) |
| F25 | P1 | verified | Frontend experience lead with hiring domain lead | Full-platform domain acceptance / 10A, 10B, 10F | Nine further reachable components carried the same fabrications and were missed by the F24 audit, whose name list did not include the recruiter personas. Two were more than display: the interview scorecard arrived pre-filled with 5-of-5 scores, a Strong Hire recommendation and a written assessment of a candidate the interviewer had never met, and submitting it recorded nothing while announcing success; the notes panels attributed a recruiter's own note to a colleague who does not exist and never sent it. **Fixed 9 Sep 2026.** [Batch 6 acceptance](../BATCH6_RELEASE_ACCEPTANCE_2026-09-09.md) |
| F26 | P1 | verified | Backend hiring domain lead | Full-platform domain acceptance / 10A, 10B | The fabrication was on the server too. The candidate engine named `postgresql-tsvector-v2` was constructed with a literal `nil` pool, so it never reached PostgreSQL in any environment and always took its "mock candidates fallback for testing and development"; `GetCandidateDetail` returned the first search result for every id; the recruiter dashboard's counts were literals; saving a candidate returned the success value without writing. Three backend tests asserted this, one on the exact string "Sarah Chen" and the score 96. **Fixed 9 Sep 2026.** [Batch 6 acceptance](../BATCH6_RELEASE_ACCEPTANCE_2026-09-09.md) |
