...
// @Description  The client asks to pay a pending milestone into escrow. Answers 202 with a payment intent (status requires_payment) and, when the processor uses one, a checkout_url - with Stripe, a hosted Checkout page that returns the client to the contract with ?payment=success or ?payment=cancelled. This does not fund the milestone: it becomes funded only when the payment processor's signed webhook confirms the money, and the contract becomes active on its first funded milestone. 409 (FREELANCE_ESCROW_CONFLICT) while another payment for the milestone is in progress; 503 (FREELANCE_PAYMENTS_UNAVAILABLE) when this deployment has no payment processor; 502 (FREELANCE_PAYMENT_PROVIDER_ERROR) when the processor could not start the payment. Requires a valid Bearer access token.
// @Tags         Jobs
// @Produce      json
// @Param        id           path  string  true  "Contract ID"
// @Param        milestoneId  path  string  true  "Milestone ID"
